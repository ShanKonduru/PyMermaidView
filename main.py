...
from src.flowchart_builder import FlowchartBuilder, NodeShape, ArrowType, Direction, NodeStyle
from src.mermaid_utils import TemplateManager, FlowchartValidator, create_quick_flowchart
...
async def demo_advanced_features():
    ...
    payment_subgraph.add_node(FlowchartBuilder().add_process_node("process_payment", "Process Payment").nodes["process_payment"])
    payment_subgraph.add_node(FlowchartBuilder().add_decision_node("payment_ok", "Payment Success?").nodes["payment_ok"])
    
    # Add subgraph for fulfillment
    fulfillment_subgraph = builder.add_subgraph("fulfillment", "Order Fulfillment")
    fulfillment_subgraph.add_node(FlowchartBuilder().add_process_node("pick_items", "Pick Items").nodes["pick_items"])
    fulfillment_subgraph.add_node(FlowchartBuilder().add_process_node("pack_order", "Pack Order").nodes["pack_order"])
    fulfillment_subgraph.add_node(FlowchartBuilder().add_process_node("ship_order", "Ship Order").nodes["ship_order"])
